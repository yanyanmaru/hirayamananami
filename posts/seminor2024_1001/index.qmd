...
$Y=y_i$ を与えた時の$X$の条件付き確率変数 $p_{X|Y}(x_j|y_i)$ で表すと、$Y=y_i$を与えた時の$g(X)$の条件付き期待値は
...
$Y=y$を与えた時の$X$の条件付き平均$E[X|Y=y]$

条件付き分散 $Var[X|Y=y]=E[(X-E[X|Y=y])^2|Y=y]$ と表される。
...
\begin{align}
E[E[X|Y]]&= \sum_{i=1}^{\infty} \sum_{j=1}^{\infty} x_j p_{X|Y}(x_j|y_i) \\
&= \sum_{j=1}^{\infty} x_j \sum_{i=1}^{\infty}  p_{X|Y}(x_j|y_i) \\
