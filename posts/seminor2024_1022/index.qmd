...
$X$ はベルヌーイ分布に従う$n$ 個の独立な確率変数の和で表される[^1]ので、$n$ が大きい時は中心極限定理が適用できて、$(X-np)/\sqrt{np(1-p)}$ は標準正規分布で近似できる。[^2]

[^1]: $X_i \sim Ber(p)$ にそれぞれ従う時、$X=X_1+X_2+ \cdots + X_n$ とすると、和の分布である$X$ は二項分布に従う。p18の定義にも書いてある。

[^2]: 二項分布に従う確率変数$X$の$E[X]=np$,$V[X]=np(1-p)$より。
...
のように近似し、標準正規分布の分布関数 $\Phi(\cdot)$ を用いて確率を計算することができる。下の図は教科書の図6.2を再現している。
